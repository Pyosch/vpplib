...
sb_code = "1-MVLV-semiurb-all-0-sw" # "1-LV-semiurb4--0-sw"
...
print(time.asctime(time.localtime(time.time())))
print("Initialized vpp\n")
# %% include Simbench network

net = sb.get_simbench_net(sb_code)

#%% drop buses

len_before_drop = len(net.bus.name)

# get unique lv subnets from grid
lv_subnet_lst=list()
for subnet in net.bus.subnet.unique():
    # some subnets consist of the combination of lv and mv subnet names
    # by filtering out mv grids mv-lv and hv-mv subnets are excluded
    if "MV" not in subnet:
        lv_subnet_lst.append(subnet)

# choose subnets to drop from grid
drp_subnet_lst = random.sample(lv_subnet_lst, 100)

# drop buses from grid
# pp.drop_buses() expects a list, therefore put net.bus.index in []
for bus in net.bus.name:
    for subnet in drp_subnet_lst:
        if subnet in bus:
            pp.drop_buses(net, [net.bus.index[net.bus.name == bus].item()])

len_after_drop = len(net.bus.name)
lv_bus_lst = list()
for bus in net.bus.name:
    if "LV" in bus:
        lv_bus_lst.append(bus)

print("Grid reduced from ", len_before_drop, " to ", len_after_drop, " buses.")
print(len(lv_bus_lst), " lv buses in the grid.\n")

# print number of unsupplied buses
unsupplied_buses = pp.unsupplied_buses(net)
print("Number of unsupplied buses: ", len(unsupplied_buses),"\n")

print(sb_code,"\n")
print(net,"\n")
pp.plotting.simple_plot(net)

#%% include Simbench profiles
...
print(time.asctime(time.localtime(time.time())))
print("Initialized simbench network\n")
...
count = 0
while count < 5:
    results['load_p_mw'][results['load_p_mw'].columns[count]].plot(figsize=(16,9), legend=True)
    count += 1
plt.show()
