...
environment = Environment(timebase=timebase, timezone='Europe/Berlin',
...
pv = Photovoltaic(unit="kW", identifier=(identifier + '_pv'),
...
df_component_values["pv_kWp"] = pv.module.Impo * pv.module.Vmpo / 1000 *pv.system.modules_per_string *\
...
bev = BatteryElectricVehicle(identifier=(identifier + '_bev'),
...
hp = HeatPump(identifier=(identifier + '_hp'),
              environment=environment, user_profile=user_profile,
...
#    df_timeseries.heat_pump.plot(figsize=figsize, label="heat pump [kW-el]")
    ...
    tes_hp = ThermalEnergyStorage(mass=mass_of_storage,
    ...
    hp = HeatPump(identifier='hp1',
                  environment=environment, user_profile=user_profile,
                  el_power=el_power_hp, ramp_up_time=ramp_up_time,
...
tes_chp = ThermalEnergyStorage(identifier='th_storage',
